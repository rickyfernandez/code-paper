...
\begin{figure}
    \begin{center}
        \includegraphics[width=0.5\textwidth]{figures/linear-wave-l1.eps}
...
We consider a simple shear-flow, called the Kelvin Helmholtz problem, where a single
mode is excited by a velocity perturbation. The problem consists of two layers that have
different densities and are initially in pressure equilibrium. Each layer flows in the
opposing direction and receives a velocity perturbation parallel to the interface and
with opposite signs. This perturbation grows exponentially and produces structures which
are called Kelvin Helmholtz instabilities. A difficulty of this problem is the convergence
...
We follow Springel and setup a box of unit length with density
\begin{equation}
	\rho = \left\{
      \begin{array}{@{}ll@{}}
            2 & \text{for}\ y < 0.25 \\
