...
Our \lstinline{DomainManager} class is designed to handle all information associated with
...
In parallel, each spatial region associated with a processor is considered
a domain. The two most important tasks of the \lstinline{DomainManger} is summarized below:
\begin{itemize}
	\item Create ghost particles.
    \item Transfer data across boundaries.
\end{itemize}
...
of the global Voronoi mesh is delegated to the construction of a disjoint set
...
boundary then it is flagged and a ghost particle is created and exported to that processor. 
Care is taken such that no duplicates are created through the whole process. After each
particle is checked all ghost particles are communicated and the mesh is allowed to updated itself.
This procedure is repeated until no more particles are flagged.
...
with \lstinline{DomainManager}.

1. during the course of mesh creation the mesh flags bad particles
2. the boundary passes those bad particles to the boundary condition
3. new particles are created and the boundary gives them back to the mesh
4. process repeat until the mesh has converged
5. domain manager responsible to sort and keep track of which particles
6. are ghost.

\subsubsection{Data Communication}
The \lstinline{DomainManager} keeps track of all domains. 
3. the domain manager helps finding these ghost particles
4. after created the domain manager is responsible in transfering data
The detail of ghost creation will be discussed in Sections. After the creation
of ghost particles, supplemental data has to be communicated. For example, the
center of mass can not be computed locally for the ghost particle since it is
not guaranteed that all neighbors of the ghost particle exist locally. Our ghost
particle implementation only guarantees that all particle 

\subsubsection{Particle Motion}
1. after particle movement particles need to the correct point in space
2. for particle that leave the domain the boundary condition handles it
3. in parallel leaving a patch domain has to be communicated
